...
- **Session 13 re-expresses the estimation flow as an explicit LangGraph `StateGraph`** (`app/domain/graph/`), instead of the hand-rolled loop of Session 12 — the "domain conductor" is the right home per this doc's own rule that cross-cutting orchestration lives in `domain/`, not inside a `generation` sibling. `state.py` defines `EstimationState` (a plain `TypedDict`) with two **accumulator** fields (`Annotated[list[...], operator.add]`): `budget_matches` and `errors` — each node returns only its own partial update and LangGraph merges it onto whatever prior nodes already contributed. Five **sequential**, pure async nodes (`nodes.py`): `extract_requirements` → `classify_components` (both via `LLMWrapper.complete_structured`, model from `AGENT_MODEL` — `reasoning_effort` is only passed for the gpt-5 family, since litellm maps it to Claude's "thinking", which conflicts with Instructor's forced tool-calling) → `search_budgets` (one component at a time — parallelising this, via LangGraph's `Send` API, is live-session work) → `generate_estimate` (median + `CONTINGENCY_FACTOR=0.15` per component, same rule as Session 12) → `validate_and_consolidate` (sets `status`). `build.py` wires them with a **conditional edge** (`add_conditional_edges` keyed on `status`, not a fixed edge to `END`) — both branches currently land on `END`; the live session gives `"needs_review"` a real recovery target. **Persistence**: `checkpointer.py` opens an `AsyncPostgresSaver` on the project's own Postgres (`plain_postgres_dsn()` strips the SQLAlchemy driver token off `DATABASE_URL`; `AsyncPostgresSaver` manages its own tables, created by `checkpointer.setup()` on first use) — no new infrastructure, and `app/main.py`'s `lifespan` owns it via an `AsyncExitStack` so `POST /v1/estimate/graph` (`app/api/routers/estimate_graph.py`, same auth/rate-limit shape as `estimate.py`) gets a 503 instead of crashing if Postgres is unreachable. **Observability**: `observability.py::configure_logfire()` — idempotent, called once at import time in `main.py` — wraps every node body in `logfire.span(...)` and instruments FastAPI + httpx (LiteLLM's calls ride on httpx; `instrument_litellm()` needs an extra `openinference` dependency not worth adding here). Works with zero Logfire account: `send_to_logfire="if-token-present"` plus an explicit non-verbose `ConsoleOptions` print every span locally (verbose mode's box-drawing detail panels crash on a non-UTF-8 console codepage, e.g. Windows cp1252). **Windows-specific gotcha**: `psycopg`'s async mode cannot run on the default `ProactorEventLoop`. `scripts/run_graph_s13.py` sets `asyncio.WindowsSelectorEventLoopPolicy()` at import time on `win32` (its own process, harmless elsewhere) — but `app/main.py` deliberately does NOT set this globally: doing so once destabilized the wider pytest suite (a multi-minute hang across unrelated tests importing `app.main`), so on a bare Windows host (not Docker) the checkpointer setup in `lifespan` fails and is caught by the existing try/except, degrading to `app.state.graph = None` (503 on `/v1/estimate/graph`, the rest of the service unaffected) — Docker/Linux is unaffected either way. Run standalone via `scripts/run_graph_s13.py` (`--model`/`--stub`/`--thread-id`/`--no-checkpoint`/`--out`); `--stub` swaps in `exercises/session-12/reference_retrieval.py`'s offline corpus when the real pipeline's embeddings dependency isn't available. Tests (`tests/domain/graph/`) are network-free — LLM calls and the retrieval backend are mocked; `test_state.py` builds a tiny throwaway graph just to prove the accumulator reducers merge correctly.
...
Session 13 vars: reuses `AGENT_MODEL` / `AGENT_REASONING_EFFORT` (the graph's `extract_requirements`/`classify_components` nodes) and `AGENT_SEARCH_TOP_K` / `AGENT_SEARCH_DISTANCE_THRESHOLD` (the `search_budgets` node) — no new settings this session. `DATABASE_URL` doubles as the checkpointer's connection string (driver token stripped by `plain_postgres_dsn()`).
