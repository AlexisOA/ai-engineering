...
- **Session 12 adds a hand-rolled agent loop over the raw Responses API** (`app/generation/agentic/agent_loop.py`) — the **one deliberate exception** to "everything goes through `LLMWrapper`": the exercise is precisely about the Responses API's own `function_call`/`call_id`/`previous_response_id` mechanics, which an Instructor/LiteLLM abstraction would hide. `run_agent()` drives the loop manually (`client.responses.create` → walk `response.output` for `function_call` items → dispatch via `app/generation/agentic/agent_tools.py::dispatch_tool` → return `function_call_output` chained on `previous_response_id` → repeat until no more tool calls or `AGENT_MAX_ITERATIONS`, then one `client.responses.parse(text_format=AgentEstimate)` for the structured result). Two (+1) tools, defined as **flat** `strict:true` JSON schemas (`TOOL_SCHEMAS`, the Responses API shape — not nested under `function` like Chat Completions): `search_budgets` (wraps the real S9–S10 `retrieve()`, filtered to `chunk_type='historical_task'`) and the deterministic, LLM-free `calculate_estimate`/`validate_estimate` (median + `CONTINGENCY_FACTOR=0.15`, empty references flagged `unbudgeted` not invented). **Architectural note**: `agentic/agent_tools.py` does NOT import `app.generation.rag` directly (`generation/<x>/*` may not import a `generation` sibling per this doc) — the retrieval backend is injected as a callable (`RetrievalBackend`, the same style `boss.py` already injects `ActorCallable`/`CriticCallable`), wired for real in the composition root (`app/dependencies.py::get_agent_retrieval_backend`). Materials for the exercise (sample transcripts, the offline `reference_retrieval.py` safety-net stub) live in `estimator/exercises/session-12/`; `scripts/run_agent_s12.py` is the real entrypoint (`--stub` swaps in the offline stub, no DB needed) and `scripts/run_agent_s12_fallback.py` is a documented one-off (Anthropic's tool-use API + the offline stub) used only while OpenAI credit is unavailable — see its docstring.
...
Session 12 vars (agent loop):
- `AGENT_MODEL` / `AGENT_REASONING_EFFORT` — default `gpt-5` / `medium`. Debug the loop mechanics cheaply with `--model gpt-5-mini --effort minimal` on the CLI before a real run.
- `AGENT_MAX_ITERATIONS` — default `8`. Hard safeguard independent of the loop's natural termination.
- `AGENT_SEARCH_TOP_K` / `AGENT_SEARCH_DISTANCE_THRESHOLD` — default `5` / `0.6`, passed straight to `retrieve()` for the `search_budgets` tool.
